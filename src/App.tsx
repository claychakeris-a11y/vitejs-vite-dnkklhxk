...
const PLANS = {
  basic: {
    ...
    stripe: STRIPE.basic,
    features: [
      "✅ Instant tier ratings (S through F)",
      "✅ Resale prices across 7 platforms",
      "✅ Max buy price recommendations",
      "✅ Previously rated item history",
      "❌ Photo upload",
      "❌ Profit estimates",
      "❌ Direct listing links",
      "❌ Expert reseller tips",
    ],
    ratingLimit: 5,
    hasPhoto: false,
    hasProfit: false,
  ...
  pro: {
    ...
    features: [
      "✅ Everything in Basic",
      "✅ Photo upload for AI visual analysis",
      "✅ Profit estimates after fees",
      "✅ Direct links to list on every platform",
      "✅ Unlimited ratings",
      "✅ Condition-based pricing tips",
      "❌ Priority new features",
      "❌ Advanced expert tips database",
    ],
    ratingLimit: Infinity,
    hasPhoto: true,
    hasProfit: true,
    hasTips: false,
    hasLinks: true,
  ...
  elite: {
    ...
    features: [
      ...
      "✅ Advanced condition-based pricing",
      "✅ Full expert tips database",
...
// Expert tips database — Elite only
const EXPERT_TIPS = {
  S: [
    "List within 24 hours of purchase — hype fades fast and early sellers get the premium prices.",
    "Always photograph every angle including tags, soles, and any flaws — buyers will ask anyway.",
    "Cross-list on StockX AND eBay simultaneously for maximum exposure.",
  ],
  A: [
    "Research the last 10 completed eBay sales before setting your price.",
    "Offer a bundle discount if you have multiple pieces from the same brand.",
    "List on a Thursday evening — weekend browsers are your biggest buyers.",
  ],
  B: [
    "Clean and steam the item before photographing — presentation adds 20-30% to perceived value.",
    "Use natural lighting for photos — it makes colors pop and looks more professional.",
    "Accurate measurements in the listing reduce returns significantly.",
  ],
  C: [
    "Price slightly below comps to move inventory faster — holding costs eat into margins.",
    "Consider bundling with similar items to increase average order value.",
    "Promote your listing on Depop and Poshmark using their built-in sharing tools daily.",
  ],
  D: [
    "Only flip if your cost was under $3 — margins are too thin otherwise.",
    "Bundle multiple D-tier items together to make the listing worth the buyer's shipping cost.",
    "Consider donating and using the tax write-off instead of spending time listing.",
  ],
  F: [
    "Leave it — your time is worth more than the $2 profit.",
    "Exception: if it's a collab piece or has unusual branding, research before passing.",
    "Focus your energy on S and A tier finds instead.",
  ],
};
...
const CONDITION_LABELS = {
  deadstock: "💎 Deadstock / New",
  likenew: "✨ Like New",
  good: "👍 Good Used",
  worn: "👎 Visible Wear",
  damaged: "⚠️ Stains / Damage",
};
...
// Apply condition multiplier to a price string like "$50–$150"
...
export default function App() {
  ...
  function rateItem() {
    const q = query.trim() || "clothing item in photo";
    if (!q && !image) return;
    if (ratingsUsed >= currentPlan.ratingLimit) {
      alert("You've reached your rating limit. Upgrade to Pro or Elite for unlimited ratings!");
      return;
    }
    setLoading(true);
    const analysis = keywordAnalyze(q);
    const multiplier = CONDITION_MULTIPLIERS[condition] || 1.0;
    const adjustedPrices = analysis.prices.map(p => ({
      ...p,
      price: applyCondition(p.price, multiplier),
    }));
    const adjustedProfit = applyCondition(analysis.profit, multiplier);
    const r = {
      id: Date.now(),
      name: q,
      imagePreview,
  ...
  if (page === "paywall") {
    return (
      <div style={{ background: "#faf7f2", minHeight: "100vh" }}>
        <style>{GS}</style>
        <div style={{ background: "#fff", borderBottom: "1.5px solid #e8e0d5", height: 54, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "32px 14px 60px" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: "clamp(28px,7vw,52px)", lineHeight: 1.05, letterSpacing: "-0.02em", marginBottom: 10 }}>
              Flip Smarter.<br /><span style={{ color: "#2d6a4f" }}>Make More Money.</span>
            </h1>
            <p style={{ fontFamily: "'Nunito', sans-serif", color: "#888", fontSize: 15, lineHeight: 1.7, fontWeight: 500, maxWidth: 420, margin: "0 auto" }}>
              Get instant tier ratings, platform prices, and profit estimates on any clothing item. Choose a plan to get started.
            </p>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {Object.entries(PLANS).map(([key, p]) => (
              <div key={key} style={{ background: "#fff", border: "2px solid " + p.color + "44", borderRadius: 18, padding: "20px 22px", position: "relative" }}>
                {key === "pro" && (
                  <div style={{ position: "absolute", top: -12, right: 20, background: p.color, color: "#fff", borderRadius: 20, padding: "3px 12px", fontFamily: "'Nunito', sans-serif", fontSize: 12, fontWeight: 800 }}>MOST POPULAR</div>
                )}
                {key === "elite" && (
                  <div style={{ position: "absolute", top: -12, right: 20, background: p.color, color: "#fff", borderRadius: 20, padding: "3px 12px", fontFamily: "'Nunito', sans-serif", fontSize: 12, fontWeight: 800 }}>BEST VALUE</div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                  ...
                  <a href={p.stripe} target="_blank" rel="noopener noreferrer"
                    onClick={() => { setPlan(key); setTimeout(() => setPage("home"), 500); }}
                    style={{ background: p.color, color: "#fff", border: "none", borderRadius: 12, padding: "11px 20px", fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: 14, display: "inline-block" }}>
                    Subscribe →
                  </a>
                </div>
                ...
                {key === "basic" && (
                  <div style={{ marginTop: 12, background: "#f0fdf4", borderRadius: 8, padding: "8px 12px", fontFamily: "'Nunito', sans-serif", fontSize: 12, color: "#16a34a", fontWeight: 600 }}>
                    ⚡ Includes 5 ratings per month
                  </div>
                )}
              </div>
            ))}
          ...
          <div style={{ textAlign: "center", marginTop: 20 }}>
            <button onClick={() => { setPlan("basic"); setPage("home"); }}
              style={{ background: "none", border: "none", fontFamily: "'Nunito', sans-serif", fontSize: 14, color: "#aaa", fontWeight: 600, textDecoration: "underline" }}>
      ...
      <div style={{ background: "#faf7f2", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 14px 80px" }}>
          ...
          <div style={{ background: T.bg, border: "2px solid "+T.color+"44", borderRadius: 20, padding: "28px 20px", textAlign: "center", marginBottom: 14 }}>
            {result.imagePreview && (
              <img src={result.imagePreview} alt="item" style={{ width: 90, height: 90, borderRadius: 14, objectFit: "cover", border: "3px solid #fff", boxShadow: "0 4px 16px #00000020", margin: "0 auto 14px", display: "block" }} />
            )}
            <div style={{ width: 88, height: 88, borderRadius: 18, background: "#fff", border: "4px solid "+T.color, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Archivo Black', sans-serif", fontSize: 48, color: T.color, margin: "0 auto 12px" }}>
              {result.tier}
            </div>
          ...
          {/* Profit + Buy — Pro/Elite only */}
          {currentPlan?.hasProfit ? (
            ...
            <div style={{ background: "#fff", border: "1.5px dashed #e8e0d5", borderRadius: 14, padding: "16px 18px", marginBottom: 14, textAlign: "center" }}>
              <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 14, color: "#aaa", fontWeight: 600, marginBottom: 8 }}>🔒 Profit estimates available on Pro & Elite</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {result.prices.map((p, i) => {
              const pl = PLATFORMS.find(x => x.name === p.name) || { emoji: "🔗", url: "https://ebay.com" };
              return currentPlan?.hasLinks ? (
          ...
          {currentPlan?.hasTips && result.tips?.length > 0 && (
          ...
          {!currentPlan?.hasTips && (
            <div style={{ background: "#fff", border: "1.5px dashed #e8e0d5", borderRadius: 14, padding: "14px 18px", marginBottom: 14, textAlign: "center" }}>
              <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 14, color: "#aaa", fontWeight: 600, marginBottom: 8 }}>🔒 Expert tips available on Elite plan</div>
          ...
          <div style={{ background: "#2d6a4f", borderRadius: 16, padding: "18px 22px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 13, color: "#a7d8bc", fontWeight: 600 }}>
                {currentPlan?.ratingLimit === Infinity ? "Unlimited ratings on your plan" : `${currentPlan?.ratingLimit - ratingsUsed} ratings left this month`}
              </div>
            </div>
    ...
    <div style={{ background: "#faf7f2", minHeight: "100vh" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1.5px solid #e8e0d5", height: 54, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {currentPlan && <span style={{ fontFamily: "'Nunito', sans-serif", fontSize: 12, color: "#aaa", fontWeight: 600 }}>{currentPlan.name} · {ratingsUsed}/{currentPlan.ratingLimit === Infinity ? "∞" : currentPlan.ratingLimit}</span>}
          <button onClick={() => setPage("paywall")} style={{ background: "#f0fdf4", border: "1.5px solid #16a34a30", borderRadius: 20, padding: "5px 12px", fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: 12, color: "#16a34a" }}>
            {currentPlan ? "Upgrade" : "Plans"}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "32px 14px 60px" }}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: "clamp(30px,8vw,60px)", lineHeight: 1.0, letterSpacing: "-0.02em", marginBottom: 8 }}>
            Is It Worth<br /><span style={{ color: "#2d6a4f" }}>The Flip?</span>
          </h1>
          <p style={{ fontFamily: "'Nunito', sans-serif", color: "#888", fontSize: 14, lineHeight: 1.7, fontWeight: 500 }}>
            {currentPlan?.name} Plan · {currentPlan?.ratingLimit === Infinity ? "Unlimited" : currentPlan?.ratingLimit - ratingsUsed + " ratings left"}
          </p>
        </div>
        ...
        <div style={{ background: "#fff", border: "1.5px solid #e8e0d5", borderRadius: 18, padding: 18, marginBottom: 20, boxShadow: "0 4px 20px #00000008" }}>
          ...
          {/* Photo upload — Pro/Elite only */}
          {currentPlan?.hasPhoto ? (
            <>
              ...
              {imagePreview ? (
                <div style={{ position: "relative", marginBottom: 12 }}>
                  ...
                  <button onClick={() => { setImage(null); setImgPrev(null); }}
                    style={{ position: "absolute", top: 8, right: 8, background: "#000000aa", border: "none", borderRadius: 20, padding: "4px 10px", color: "#fff", fontSize: 13, fontWeight: 700 }}>✕</button>
                ...
                <div className="upload-box" onClick={() => fileRef.current.click()} style={{ marginBottom: 12 }}>
            ...
            <div style={{ background: "#f9fafb", border: "1.5px dashed #e0e0e0", borderRadius: 12, padding: "14px", marginBottom: 12, textAlign: "center" }}>
              <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 13, color: "#bbb", fontWeight: 600, marginBottom: 6 }}>🔒 Photo upload available on Pro & Elite</div>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <select value={condition} onChange={e => setCondition(e.target.value)}
              style={{ width: "100%", background: "#faf7f2", border: "1.5px solid #e8e0d5", borderRadius: 10, padding: "10px 13px", fontSize: 14, color: "#1a1a1a", fontWeight: 600 }}>
          ...
          <textarea rows={2} value={query} onChange={e => setQuery(e.target.value)}
            ...
            placeholder={imagePreview ? "Add description for better results (optional)..." : '"Supreme Box Logo Hoodie" or "Vintage Carhartt jacket"'}
          ...
          <button onClick={rateItem} disabled={loading || (!query.trim() && !image)}
            style={{ display: "block", width: "100%", background: loading || (!query.trim() && !image) ? "#ccc" : "#2d6a4f", color: "#fff", border: "none", borderRadius: 10, padding: "14px", fontWeight: 800, fontSize: 16 }}>
            {loading ? "⏳ Analyzing..." : image ? "📸 Rate With Photo →" : "Rate This Item →"}
          </button>

          <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 6 }}>
            {["Supreme Box Logo","Nike Dunk Low","Vintage Carhartt","Jordan 1 Chicago","Patagonia Fleece","Levi's 501"].map(ex => (
              <button key={ex} onClick={() => setQuery(ex)}
                style={{ background: "#f5f0e8", border: "none", borderRadius: 20, padding: "5px 12px", color: "#888", fontSize: 13, fontWeight: 600 }}
          ...
          {Object.entries(TIERS).map(([id, T]) => (
            <div key={id} style={{ background: T.bg, border: "1.5px solid "+T.color+"30", borderRadius: 10, padding: "9px 11px", display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 10, color: "#999", fontWeight: 500 }}>{T.sub.split("—")[1]?.trim()}</div>
              </div>
            </div>
          ))}
        </div>
